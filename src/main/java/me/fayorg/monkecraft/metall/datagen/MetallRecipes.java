...
public class MetallRecipes extends RecipeProvider implements IConditionBuilder {
    ...
    protected void buildRecipes(Consumer<FinishedRecipe> consumer) {
        ...
        ShapelessRecipeBuilder.shapeless(RecipeCategory.FOOD, MetallItems.FLOUR.get()).requires(Tags.Items.CROPS_WHEAT).unlockedBy(getHasName(Items.WHEAT), has(Items.WHEAT)).save(consumer);
        ShapelessRecipeBuilder.shapeless(RecipeCategory.FOOD, MetallItems.DOUGH.get()).requires(Tags.Items.EGGS).requires(MetallItems.FLOUR.get(), 2).requires(Items.WATER_BUCKET, 1).unlockedBy(getHasName(MetallItems.FLOUR.get()), has(MetallItems.FLOUR.get())).save(consumer);
    }
}
